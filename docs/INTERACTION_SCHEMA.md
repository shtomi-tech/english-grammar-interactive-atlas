...
`GRAM-INT-001`〜`GRAM-INT-040` を1件ずつ保持し、10カテゴリを各4件で構成します。既存の4つのDemoは実働のまま維持し、カタログ拡張で新しいDemoを自動的に増やしません。新しい項目はまず学習目的・触る対象・変化・気づきを記録し、同じ操作概念の重複や、実装されていないDemoのふりを避けます。
